...
- **追記・未コミット**: 針葉樹林の頂角をさらに小さく調整し、横線を右へ合計3移動した後、全体を約5%縮小した。桑畑は木を1つだけにし、中央へ下げて大きくした。SVG読み込みは `?v=shinyojurin-scale-v1`。

- **作業中・未コミット**: 港湾記号の曲線端に塗りつぶしの二等辺三角形を置き、三角形の重心を曲線端へ一致、二等分線を曲線端の接線と平行にした。頂角の向きは接線に対して180度反転済みで、底辺を広げて頂角を大きくした。上部の丸は半径を13から11へ縮小した。高等学校記号は横棒を外円に接する長さへ延長し、内部線を太く、斜線の下端も外円まで延長したうえで、斜線同士の開きを少し狭め、上端を横棒と重ねて接続した後、横棒と斜線で囲まれる白い逆三角形を広げた。小・中学校記号は斜線の上端を横棒内へ少し下げ、butt端にして上への角のはみ出しをなくしつつ、余白が出ない位置へ再調整した。針葉樹林は左右の斜線を一本の連続パスにして頂点の余白をなくし、広葉樹林の外接サイズに揃えた後、頂角をさらに細くし、線幅を6.5へ下げ、横線を左へ2移動した後に右へ1戻した。広葉樹林も線幅を6.5に揃え、下の横線を左右からさらに2ずつ短くした。温泉記号は3本の湯気線の横間隔を少し詰め、老人ホームは中央の杖を正しい向き（左は短い持ち手、右が床まで伸びる杖）に修正し、さらに建物中央へ2だけ左寄せした。SVG読み込みは `?v=forest-lines-v3` に更新済み。Edgeで描画確認済みで、ユーザー確認後にのみコミット・デプロイする。
...
### 2026-07-27 — Codex（ローカル変更・未コミット／未デプロイ）
...
### 2026-07-27 — Codex（ローカル試行・未コミット／未デプロイ）
...
- 追加調整（未コミット）: 学校・高等学校の上点を垂直線に統一。郵便局は文字フォントを使わず、円に接する同一太さの線で〒を描画。市役所の内円は細線化し、博物館のひさしを短縮した。
- 警察署・広葉樹林・桑畑・田・畑・老人ホーム・図書館・温泉・果樹園の線端と角を角形に統一し、不要なはみ出しと隙間を調整した。茶畑も「3粒の単位記号」を3つ三角配置に変更した。
- 再調整（未コミット）: 郵便局の長い横線を下げ、学校の斜線を左右対称にした。高等学校の横線端は円との接続で角が出ないようにした。図書館は以前の開いた本の形へ戻した。
- 記念碑の右側だけに台座の出を追加。病院の五角形を横長にし、十字を外枠から離した。老人ホームは屋根から出る壁線をなくして、杖を左右とも床へ届く細線に描き直した。針葉樹林は公式の細い三角形と下辺に戻し、広葉樹林は幹を樹冠の接点まで伸ばした。
- 再々調整（未コミット）: 学校・高等学校の上線を横棒まで接続。郵便局は上下の横・縦線を円の接点まで伸ばした。記念碑には下辺を追加し、針葉樹林の下辺は右半分だけにした。桑畑は単位記号を3つ三角配置にした。
- 温泉は湯船を横に広げ、湯気を少し大きく波立たせた。老人ホームは、屋根を壁より張り出させつつ壁の端を屋根より上へ出さず、杖の左・右とも床線へ接続した。
- 再々々調整（未コミット）: 針葉樹林は細い三角形の**最下部の横棒**を右へ大きくずらし、右の斜線から明確に出す形にした。ほかの線端は余計に張り出さないよう butt 指定にした。温泉の湯船は約240度の円弧へ変更。桑畑の3単位記号はさらに小さくした。
- 再々々々調整（未コミット）: 温泉は正面からの円弧ではなく、奥行きを表す横長の楕円弧（約240度）に変更。高等学校は上線を内側の円へ接続し、横線端は円の内縁で止めて角が出ないよう再調整した。
- 再々々々々調整（未コミット）: 温泉の湯気3本を下へ移動。高等学校の上線は縦線を維持し、横線端だけを外円の前面描画で隠す方式に変更した。
