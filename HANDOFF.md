...
- **コンテンツ**: 本番は `quizzes` 8件 / `items` 384問。現役は6クイズ290問（日本地理107・歴史63・都道府県(地図)47・世界遺産26・地形25・地図記号22）、旧地図クリック式2本94問は `is_archived=1`。分野は地理と歴史の2つ。
...
- 既存DBへの変更は `migrations/` にファイルを追加して適用する。既存の連番は `0001_add_quizzes_and_nihon_chiri.sql` / `0002_fix_literal_newlines.sql` / `0003_add_chizu_kigou.sql` / `0004_add_rekishi.sql`(いずれも適用済み)。冪等にするため全INSERTを `INSERT OR IGNORE` で書く方針。
