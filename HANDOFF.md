...
- **最新コミット**: 地図記号の修正と追加（`main` へpush済み、Cloudflare Pagesの自動デプロイ対象）
...
- **コンテンツ**: 本番は `quizzes` 8件 / `items` 772問。**1行=1問**なので items の件数がそのまま問題数。現役は6クイズ678問（歴史388・日本地理107・世界遺産78・都道府県(地図)47・地図記号33・地形25）、旧地図クリック式2本94問は `is_archived=1`。分野は地理と歴史の2つ。
