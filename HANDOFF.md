...
- **最新コミット**: 1行=1問へのデータモデル変更（`main` へpush済み、Cloudflare Pagesの自動デプロイ対象）
...
- **コンテンツ**: 本番は `quizzes` 8件 / `items` 761問。**1行=1問**なので items の件数がそのまま問題数。現役は6クイズ667問（歴史388・日本地理107・世界遺産78・都道府県(地図)47・地形25・地図記号22）、旧地図クリック式2本94問は `is_archived=1`。分野は地理と歴史の2つ。
...
- 項目ごとの別解は `data/items_*.csv` の `accept` 列に `|` 区切りで書く（例: `リアス式海岸|リアス`）。`extra_json.accept` として保存され、その項目の答えを問う問題にだけ効く。
