...
- **最新コミット**: トップの並べ替えと色の凡例追加（`main` へpush済み、Cloudflare Pagesの自動デプロイ対象）
- **トップ画面**: 上から 学習を始める / レベル・連続日数・総合スコア / 今週の進み具合 / 学習する / 習熟度一覧 の順。教科 → 分野 → 個別クイズのネスト型「習熟度一覧」を実装済み。個別項目はコンパクトなタイルで色を確認できる。
- **習熟度の色**: 8段階（グレー〜赤）＋文言。凡例はトップの「習熟度一覧」の直上と `genres/` の2か所に出す（どちらも `TierScale.renderLegend()`）。
