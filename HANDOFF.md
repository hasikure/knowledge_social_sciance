...
- **最新コミット**: 先生のプレイを生徒の記録から分離＋トップの学習開始を選択式に（`main` へpush済み、Cloudflare Pagesの自動デプロイ対象）
...
- **コンテンツ**: 本番は `quizzes` 21件 / `items` 1012問。**1行=1問**なので items の件数がそのまま問題数。現役は19クイズ918問。
  - 社会6クイズ678問（歴史388・日本地理107・世界遺産78・都道府県(地図)47・地図記号33・地形25）。分野は地理と歴史。
